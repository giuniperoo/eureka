  ...
  var emojify = (function () {
    ...
    return {
      ...
      initialize: function (config) {
        this.config.cdn_host              = typeof config.cdn_host              !== 'undefined' ? config.cdn_host              : this.config.cdn_host;
        this.config.root_element          = typeof config.root_element          !== 'undefined' ? config.root_element          : this.config.root_element;
        this.config.emojify_tag_type      = typeof config.emojify_tag_type      !== 'undefined' ? config.emojify_tag_type      : this.config.emojify_tag_type;
        this.config.emoji_image_extension = typeof config.emoji_image_extension !== 'undefined' ? config.emoji_image_extension : this.config.emoji_image_extension;

        if (!config.emoticons_enabled) { this.config.selected_sets.push(EMOTICONS); }
        if (!config.people_enabled   ) { this.config.selected_sets.push(PEOPLE);    }
        if (!config.nature_enabled   ) { this.config.selected_sets.push(NATURE);    }
        if (!config.objects_enabled  ) { this.config.selected_sets.push(OBJECTS);   }
        if (!config.places_enabled   ) { this.config.selected_sets.push(PLACES);    }
        if (!config.symbols_enabled  ) { this.config.selected_sets.push(SYMBOLS);   }
      },

      run: function (el) {
        var el = el ? el : this.config.root_element,
            idx, subIdx, emoji, wrap;

        for (idx = 0; idx < this.config.selected_sets.length; idx += 1) {
          for (subIdx = 0; subIdx < this.config.selected_sets[idx].length; subIdx += 1) {
            emoji = this.config.selected_sets[idx][subIdx];
            findText(el, emoji[0], function (node, match) {
              wrap = document.createElement(this.emojify.config.emojify_tag_type);
              wrap.setAttribute('class', emoji[1]);
              title = emoji[1].replace(/emojify /g, '');
